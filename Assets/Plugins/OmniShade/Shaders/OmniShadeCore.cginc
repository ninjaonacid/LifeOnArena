...
sampler2D _LightmapDirectionTex;
...
//////////////////////////////////////////////////////////////////////////////////////////
	...
	struct v2f {
		...
		float3 nor_world : TEXCOORD2;
		#if SPECULAR && !NORMAL_MAP && !(DIRLIGHTMAP_COMBINED || LIGHT_MAP_DIR) && !FLAT
			...
			#endif
		#endif
		#if RIM && REFLECTION && !NORMAL_MAP && !FLAT
			float3 viewReflectDir : TEXCOORD6;
		#endif
		#if NORMAL_MAP && ((DIFFUSE && !LIGHTMAP_ON) || RIM || SPECULAR || MATCAP || DIRLIGHTMAP_COMBINED || (LIGHT_MAP && LIGHT_MAP_DIR)) \
			|| SPECULAR_HAIR
	...
	struct v2f {
		...
		float3 pos_world : TEXCOORD1;
		float3 nor_world : TEXCOORD2;
		#if SPECULAR && !(NORMAL_MAP || NORMAL_MAP_TOP) && !(DIRLIGHTMAP_COMBINED || LIGHT_MAP_DIR) && !FLAT
			float3 specReflectDir : TEXCOORD3;
		#endif
		#if RIM || SPECULAR
			float3 viewDir_world : TEXCOORD4;
			...
			#endif
		#endif
		#if (RIM && REFLECTION) && !(NORMAL_MAP || NORMAL_MAP_TOP) && !FLAT
			float3 viewReflectDir : TEXCOORD6;
		#endif
		#if HEIGHT_COLORS
...
v2f vert (appdata_full v) {
	...
	#if (!TRIPLANAR && \
			NORMAL_MAP && ((DIFFUSE && !LIGHTMAP_ON) || RIM || SPECULAR || MATCAP || DIRLIGHTMAP_COMBINED || (LIGHT_MAP && LIGHT_MAP_DIR))) \
		|| SPECULAR_HAIR
	...
	#endif
	#if SPECULAR && !(NORMAL_MAP || NORMAL_MAP_TOP) && !(DIRLIGHTMAP_COMBINED || LIGHT_MAP_DIR) && !FLAT && !SPECULAR_HAIR
...
half4 frag (v2f i) : COLOR {
	...
	// Calculate intermediate vectors
			...
			#endif
			#if (DIFFUSE && !LIGHTMAP_ON) || RIM || SPECULAR || MATCAP || DIRLIGHTMAP_COMBINED || (LIGHT_MAP && LIGHT_MAP_DIR)
		...
		#endif
	#endif
	#if LIGHT_MAP || LIGHT_MAP_DIR  // Lightmap UV
	...
	#endif
	#if ((NORMAL_MAP || NORMAL_MAP_TOP) || SPECULAR) && (DIRLIGHTMAP_COMBINED || LIGHT_MAP_DIR)
		#if DIRLIGHTMAP_COMBINED
			half4 lightmapDir = UNITY_SAMPLE_TEX2D_SAMPLER(unity_LightmapInd, unity_Lightmap, lightmapUnityUV);
		#elif LIGHT_MAP_DIR
			half4 lightmapDir = tex2D(_LightmapDirectionTex, lightmapUV);
		#endif
	#endif
	...
	col_base = ColorBrightness(col_base, _Brightness, _Color);
		...
		col_base.rgb += col_base_top.rgb;
			...
			i.color = pow(i.color, _VertexColorContrast);
		#endif
		col_base *= i.color;
	#endif
	#if DETAIL
	...
	// LIGHTING
	...
	#if LIGHT_MAP
		#if LIGHT_MAP_HDR
			half3 col_light = DecodeHDR(tex2D(_LightmapTex, lightmapUV), _LightmapTex_HDR);
		#else
		...
		#if LIGHT_MAP_DIR && NORMAL_MAP
			col_light = DecodeDirectionalLightmap(col_light, lightmapDir, bumpNor_world);
		#endif
		col *= col_light;
	#endif
	#if LIGHTMAP_ON
		...
		col = BlendMatCap(col.rgb, col_matcap);
	#endif
	#if SPECULAR
		#if !DIRLIGHTMAP_COMBINED && !LIGHT_MAP_DIR
			...
			float2 specUV = uv;
				...
				specUV = uv2;
			#endif
			half3 col_specular = tex2D(_SpecularTex, TRANSFORM_TEX(specUV, _SpecularTex)).rgb;
			half shiftTex = col_specular.r - 0.5;
		#else
			half3 col_specular = 1;
			half shiftTex = 0;
		#endif
		#if !SPECULAR_HAIR
			#if (NORMAL_MAP || NORMAL_MAP_TOP) || DIRLIGHTMAP_COMBINED || LIGHT_MAP_DIR || FLAT
		...
		#endif
		glare = pow(glare, _SpecularSmoothness); 
		#if SPECULAR_HAIR
		...
		#endif
		col_specular *= glare * _SpecularBrightness * specColor;
		col += col_specular;
	#endif
	#if RIM 
		...
		rim = min(1, pow(rim, _RimContrast) * _RimAmount) * col_rim.a;
			...
			#endif
			float3 col_reflect = DecodeHDR(texCUBE(_ReflectionTex, viewReflectDir), _ReflectionTex_HDR);
			col_reflect *= _ReflectionBrightness * col_rim.rgb;
			col = lerp(col, col_reflect, rim);  // Reflection use alpha blend
		#else
