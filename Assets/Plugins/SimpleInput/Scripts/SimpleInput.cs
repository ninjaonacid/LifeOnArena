...
public class SimpleInput : MonoBehaviour
{
    ...
    private static readonly Dictionary<string, Axis> axes = new Dictionary<string, Axis>();
    private static readonly List<Axis> axesList = new List<Axis>();

    private static readonly List<AxisInput> trackedUnityAxes = new List<AxisInput>();
    private static readonly List<AxisInput> trackedTemporaryAxes = new List<AxisInput>();

    private static readonly Dictionary<string, Button> buttons = new Dictionary<string, Button>();
    private static readonly List<Button> buttonsList = new List<Button>();

    private static readonly List<ButtonInput> trackedUnityButtons = new List<ButtonInput>();
    private static readonly List<ButtonInput> trackedTemporaryButtons = new List<ButtonInput>();

    private static readonly Dictionary<int, MouseButton> mouseButtons = new Dictionary<int, MouseButton>();
    private static readonly List<MouseButton> mouseButtonsList = new List<MouseButton>();

    private static readonly List<MouseButtonInput> trackedUnityMouseButtons = new List<MouseButtonInput>();
    private static readonly List<MouseButtonInput> trackedTemporaryMouseButtons = new List<MouseButtonInput>();

    private static readonly Dictionary<KeyCode, Key> keys = new Dictionary<KeyCode, Key>();
    private static readonly List<Key> keysList = new List<Key>();
    ...
    private void Update()
    {
        ...
        var sensitivity = GetAxisSensitivity * (GetAxisTimeScaleDependent ? Time.deltaTime : Time.unscaledDeltaTime);
        ...
        for (var i = 0; i < axesList.Count; i++)
        {
            ...
            axis.valueRaw = 0f;
            for (var j = axis.inputs.Count - 1; j >= 0; j--)
            {
                var input = axis.inputs[j];
                if (input.value != 0f)
                {
 ...
 Mathf.MoveTowards( axis.valueRaw <= 0 || !GetAxisSnapValue ? axis.value : 0, axis.valueRaw, sensitivity );
        ...
        for (var i = 0; i < buttonsList.Count; i++)
        {
            ...
            var isDown = false;
            for (var j = button.inputs.Count - 1; j >= 0; j--)
            {
                var input = button.inputs[j];
                if (input.value)
                {
                    isDown = true;
                    break;
        ...
        {
            var mouseButton = mouseButtonsList[i];

            var isDown = false;
            for (var j = mouseButton.inputs.Count - 1; j >= 0; j--)
            {
                var input = mouseButton.inputs[j];
                if (input.value)
                {
                    isDown = true;
                    break;
        ...
        for (var i = 0; i < keysList.Count; i++)
        {
            ...
            var isDown = false;
            for (var j = key.inputs.Count - 1; j >= 0; j--)
            {
                var input = key.inputs[j];
                if (input.value)
                {
                    isDown = true;
                    break;
